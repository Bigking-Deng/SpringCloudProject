...
v_project_name=WBXmatsfraudbackendservice
v_rmc_dir=\fraud/rhel8_64/
...
#cd /opt/webex/dev3/cloudtools-mats/WBXmatsfraudbackendservice
...
if [[ ${p_buildnumber} == "" ]]; then
      ...
      LATEST_MAVEN_BUILD_NUMBER=`sudo su -c 'yum list WBXmatsfraudbackendservice-'${p_version} | sed -n '$p' | sed 's/\s\+/ /g' | cut -d' ' -f 2 | cut -d'-' -f 2 | cut -d'_' -f 2`
      echo "LATEST_MAVEN_BUILD_NUMBER is ${LATEST_MAVEN_BUILD_NUMBER}"
      if [[ ${LATEST_MAVEN_BUILD_NUMBER} == "*" ]]; then
            p_buildnumber=$now_date'_1'
      else
            p_buildnumber=$now_date'_'$((${LATEST_MAVEN_BUILD_NUMBER}+1))
      fi
fi
...
v_redhat8="-Djavax.net.debug=all -Djavax.net.ssl.keyStore=/opt/WBXlib-fips-jar/ssl/os.server.keystore.bcfks -Djavax.net.ssl.keyStorePassword=testfips -Djavax.net.ssl.trustStore=/opt/WBXlib-fips-jar/ssl/os.server.truststore.bcfks -Djavax.net.ssl.trustStorePassword=testfips -Djavax.net.ssl.keyStoreType=BCFKS -Djavax.net.ssl.trustStoreType=BCFKS"
#mvn ${v_redhat8} clean package -Dmaven.rpm.phase=package -Dmaven.build.number=${p_buildnumber} -Drpm.build.version=${p_version} -Dmaven.test.skip=true
